...
import defense_alg.outlier_removal as outlier_removal
import defense_alg.PCA as PCA
import defense_alg.subsets as subsets
...
def generate_main(config):
    ...
    output_bag_path = Path(config['mitigate_params']['output_rosbag'])
        ...
        with Writer(output_bag_path) as writer:
            ...
            for connection, timestamp, rawdata in reader.messages(connections=connections):
                ...
                if connection.topic == imu_topic:
                    ...
                    n_subset = config["mitigate_params"]["n_subset"]
                    d_th = config["mitigate_params"]["PCA_distance_threshold"]

                    current_subsets = subsets.separate_points(written_cloud, n_subset=n_subset)

                    if prev_subsets is None:
                        prev_subsets = current_subsets
                    else:
                        # 1. subset split & matching
                        transform_matrices = subsets.subset_scan_matching(
                            source_subsets=current_subsets, 
                            target_subsets=prev_subsets
                        )

                        # 2. Run PCA
                        pose_pca, evr = PCA.pca_main(transform_matrices)

                        # 3. Outlier removal
                        kde = outlier_removal.kde_2d(pose_pca)
                        kde_peak_x, kde_peak_y = outlier_removal.get_KDEpeak(kde, pose_pca)

                        written_cloud = outlier_removal.euclidian_distance_filter(
                            current_subsets, pose_pca, kde_peak_x, kde_peak_y, distance_threshold=d_th
                        )
    ...
    print(f"\n[Success] Finished creating mitigated rosbag!")
